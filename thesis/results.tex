...
%% For a simple scene, such as the Cornell Box with 3 nodes, the exhaustive ray
%% tracer outperformed both the kd-restart ray tracer and short-stack
%% implementation, even without accounting for the time it took to construct the
%% kd-tree. This is not entirely unexpected, since the exhaustive ray tracer only
%% needs to intersect every ray with 36 triangles, where the hierarchical ray
%% tracers need to first traverse the root node before they can begin ray/triangle
%% intersection in leafs that can reference up to 32 triangles. The short-stack ray
%% tracer performs even worse than kd-restart, since it not only has to traverse
%% the tree, but also maintain its internal stack, which is of little use in such a
%% short kd-tree.


% Overall best ray tracer, which shall be used further on.
