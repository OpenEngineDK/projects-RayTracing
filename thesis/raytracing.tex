...
\refsection{sec:shortStack}. The generel algorithm for traversing a
...
\begin{algorithm}
  ...
  \begin{algorithmic}
    \PROCEDURE{Traversal}
              {$ray$ : Ray, $tree$ : kd-tree}
              {$leaf$ : Node, $t_{split}$ : Number}{
                \ASSIGN{$t_{split}$}{$\infty$}
                \ASSIGN{$node$}{$tree.root$}
                \WHILE{$node \neq LEAF$}
                  \ASSIGN{$t_{node}$}{($node.splitValue - ray.origin[node.axis]) / ray.direction[node.axis]$}
                  \IF{$0 < t_{split}$}
                    \ASSIGN{$node$}{$ray.direction[node.axis] > 0$ ? $node.left$ : $node.right$}
                    \ASSIGN{$t_{split}$}{min$(t_{split}, t_{node})$}
                  \ELSE
...
%% splitting planes is show in \reffig{fig:simpleTree}. A tree traversal
