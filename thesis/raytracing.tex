...
found in $O(\log m)$ time. For $n$ rays this yeilds a time complexity
of $O(n \log m)$.
...
\begin{figure}
  ...
  \subfloat[The kd-tree.]{
    \begin{tikzpicture}[y=0.5cm, x=.5cm,font=\sffamily,
      ...
      \node [node] (0){$x:4$}
        child {node [node] (1) {$y:4$}
          child {node [leaf] (e) {$leaf:\emptyset$}}
          child {node [node] (3) {$x:2$}            
            child {node [leaf] (e) {$leaf:\{0\}$}}
            child {node [leaf] (e) {$leaf:\{1\}$}}
          }
        }
        child {node [node] (2) {$y:2$}
            child {node [leaf] (e) {$leaf:\{2\}$}}
            child {node [leaf] (e) {$leaf:\{3\}$}}
          }
        ;
    \end{tikzpicture}
  }
