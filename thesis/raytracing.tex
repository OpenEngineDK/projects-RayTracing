...
It should be noted that rays reflected multiple times may eventually
diverge and no longer be spatially coherent, but this is no more of a
problem than with the sequential approach and the screen packets will
still speed up primary rays.
...
% Inspired by empty space splitting and combines BVH and KD tree
...
\fixme{Problem when the ray travels through a lot of empty space that
  empty space splitting doesn't pick up on, such as sponza. Then it
  goes boom!}
