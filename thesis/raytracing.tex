...
\fixme{A generel for all algorithm showing the dataparallel aspect of
  ray tracing. Then move intersection back until after ray tracing}

\fixme{Brifly mention how light and reflection is calculated. ref real
time rendering}
...
version to motivate the continued use of data structures for dynamic
geometry.
...
a packet scheme inspired by Wald et al.\citebook{Wald:2001:IRCRT} and
...
Finally I will briefly discuss shading and how reflections are
calculated.

\section{Ray/Triangle Intersection}

\fixme{Move ray/triangle intersection further down this chapter until
  after the ray tracing algorithms have been described?}
...
GPUs, providing as many early out possibilities as
...
from \refsection{sec:exhaustive}.
...
kd-tree can be seen in \refalg{alg:generelTracing}.
...
CUDA's memory model, however, isn't flexible enough for this approach,
which requires either dynamically allocating more memory if the stack
is filled or pre-allocating a \textit{large enough} stack in local
memory to handle any given kd-tree, something that could require quite
large amounts of memory for huge scenes and might even require too
...
circular stack of $N$ elements, and then revert to using kd-restart if
the stack underflows. In their research they found that a short-stack
approach only visited 3\% more nodes compared to the unlimited stack
CPU approach.
...
which will cause a significant overhead. Consequentlya short-stack
will probably not provide any speedup in this small example. But
imagine this simple tree as a subtree in a much larger scene, with
perhaps 10 levels of nodes above it. Then a short-stack implementation
...
SIMD means that the GPU is actually performing packet tracing.
...
classification seen in \reffig{fig:sequentialRayWarp} for a 64x48
...
as done in \reffig{fig:coherentRayWarp}. As the figure demonstrates
...
The calculations needed to transform sequential thread ids into
packeted ray ids are quite simple, \refalg{alg:packet}, but the
performance benefits of coherent rays and data fetches are enourmous.

It should be noted that there exists a possibility that rays reflected
multiple times may eventually diverge completely and no longer be
spatially coherent, but this is no more of a problem than with the
sequential approach and the screen packets will still speed up primary
rays.
...
% Inspired by empty space maximizing and combines BVH and KD tree
...
whose bounding box it does not even intersect. \refFig{fig:waywardRay}
...
quickly be discussed under Future Work in \refchapter{chp:future}.
...
\begin{figure}
  ...
  \subfloat{
    \begin{tikzpicture}[y=0.5cm, x=.5cm]
      ...
      \drawRay{-4,0}{-3,1}
      \draw[line width=2pt, dashed, ->, color=gray] (-3,1) -- (0,4);
    \end{tikzpicture}
