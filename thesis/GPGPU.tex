...
\fixme{Change the scan example to include leafs and next nodes? Or
  generelize to data array splitting and concretesize in kd-tree impl
  section.}
...
over the last decade. This is evidenced by \reffig{fig:gpuCpuCompare},
...
\begin{figure}
  ...
  \subfloat[floating point operations per second.]{
    \includegraphics[width=0.45\textwidth]{GFLOPS}
  }
  \subfloat[Memory bandwidth.]{
  ...
  \caption[Comparison of FLOPS and throughput on GPU's and CPU's.]{A
    ...
    from Chapter 1 in \citebook{CUDAPG}.}\label{fig:gpuCpuCompare}
\end{figure}
...
seen in \reffig{fig:gpuCpuCompare}, is that the CPU is designed with a
...
phase \reffig{fig:segScan} shows how each block's threads are
...
%% Calculating the prefix-sum however isn't the only part of the kd-tree
%% creator where reductions are needed. It is also useful for calculating
%% the bounding boxes of the kd-tree's nodes, used when performing median
%% splits.
...
this the implementation in \refalg{alg:sharedReduct} first copies the
