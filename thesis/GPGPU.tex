...
% Reduction was important in the previous step and will be again for
% median splitting

% Works on individual blocks, must be run twice if input is too large
% for one block to handle.

% Algorithms assumes that the length of the input list is a power of
% 2. If that is not the case then either the data could be
% preprocessed or the kernel itself can pad the data with the identity
% element.
...
%TODO interleaved access pattern figure
  ...
  \begin{algorithmic}
    \PROCEDURE{Reduce3}
              ...
              {\DECLARE{$sValues$}{\textbf{shared} Number List}
                ...
                \WHILE{$offset > 1$}
                  \IF{$id < offset$}
                  \ASSIGN{$rValue \leftarrow values[id]$}{\MIN{$rValue$}{$values[id + offset]$}}
                  \ENDIF
                  \ASSIGN{$offset$}{$offset / 2$}
                  \SYNC
                \ENDWHILE
...
% Only works if the number of reductions are known beforehand (or if
% we pad the input)
...
% No damn example as it's too big
