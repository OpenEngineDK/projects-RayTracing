...
\fixme{mere uddybende, FLOPS forskel, nogen hints?}
...
\reffig{fig:gpuCpuCompare}, is that the CPU is designed with a large cache at
...
of data into two lists. In \refsection{sec:kdTreeImpl} we shall see that being
...
depicted on \reffig{fig:segScan}. The figure shows scan being applied to an
...
application of $\oplus$ to the data. The reduce phase in \reffig{fig:segScan}
...
copied to both the left and right side. In \refsection{sec:upperNodes} I will
...
to the naive implementation. This shows us that knowledge of how the GPU works
