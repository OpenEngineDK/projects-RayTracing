...
\textit{hidden surface problem} and to enhance the perception of depth
by computing shadows in opaque polygonal scenes. Whitted extended the
...
to place the splitting plane he used the now standard \textit{spatial
  median splitting} algorithm. Spatial median splitting places the
splitting plane at the middle of a node's bounding box along some
axis, usually either the longest or an axis chosen in a \textit{round
  robin} fassion.
...
Popov et al.\citebook{popov:07:GPURT} in 2007 introduced a stackless
...
et al.\citebook{1230129} proposed a different but equally effective
...
%% \fixme{Remove in-depth comparisons to Horn and Zhou}
