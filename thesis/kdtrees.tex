...
\begin{figure}
  \centering \subfloat[A simple scene and its corrosponding
...
\begin{figure}
  ...
  \vspace{3mm}
  \parbox{8cm}{\caption[Excessive splitting of a triangle.]{Triangle splitting
      performing excessive splits on a triangle.}\label{fig:excessiveSplitting}}
\end{figure}
  ...
  \begin{algorithmic}
    \PROCEDURE{ChildTriangleInformation}
              ...
              {$left$ : Node, $right$ : Node}{
                ...
                \ASSIGN{$left.triangleRange$}{$leftEndAddr - left.triangleIndex$}
                ...
                \ASSIGN{$right.triangleRange$}{$rightEndAddr - right.triangleIndex$}
              }
  \end{algorithmic}
\end{algorithm}
...
\begin{figure}
  ...
  \begin{tikzpicture}[y=0.5cm, x=0.5cm]
    ...
    \draw[rounded corners=7mm, ->] (6,1) -- (8, 1.8) -- (10,1.1);
...
\begin{figure}
  ...
  \caption[A description of storing triangles as bit masks.]{A description of
    ...
    $[0,1,0,1]$. The result of splitting node 0 is leafs 1 and 2, whose triangle
    bit masks are the result of a bitwise \textbf{AND} between node 0's bit mask
    and the split candidate's left and right bit mask respectively.}
  \label{fig:bitmap}
\end{figure}
