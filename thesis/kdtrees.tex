               ...
               {\VAR{leafs,nextNodes} : Node List}{
                 ...
                 \PARALLELFOR{$segment$ \textbf{in} $segmentList$}
                   \PARALLELFOR{$triangle$ \textbf{in} $segment.triangles$}
                     \ASSIGN{$splitSide[triangle.id]$}{AssociateLeft($segment.node.splittingPlane$, $triangle$)}
                     \ASSIGN{$splitSide[triangle.id + maxTriangles]$}{AssociateRight($segment.node.splittingPlane$, $triangle$)}
                   \ENDFOR
                 \ENDFOR
