...
index and a range, $r$. The bit mask corrosponding to $r$ would then
simply have all it's first $r$ bits set. A more thorough example of
how bit masks are used is given in \reffig{fig:bitmap}.
...
Since the splitting planes are local to the lower subtree, they too
can benefit form the bit mask representation. The splitting plane
...
associating a triangle with a new node becomes trivial. I.e a new left
child's triangle set would simply be the bitwise AND of its parent's
triangle set with the splitting plane's left triangle
...
The method for precomputing the splitting planes is shown in
