...
overlaps in \refsection{sec:splittingSchemes}.
...
quality of the kd-trees in \refchapter{chp:rayTracing}.
...
ConstructKDTree takes as arguments a list of triangles and a bounding volume
...
Inspite of the assumptions made by SAH's about ray distribution and that in
...
Unfortunatly, the percentage of a node that should be empty space before it is
...
\reffig{fig:splittingCase2} shows one of the vertices directly intersected by
...
The generel kd-tree construction method presented in \refalg{alg:kdTreeCreator}
...
\refalg{alg:kdTreeCreator} must be restructured to work on multiple nodes at a
...
% SAH assumes that each split results in 2 leaf nodes, which is practically
...
\refsection{sec:reduce}. Unfortunatly there can be hundreds of thousand of
...
With the left/right and prefix-sum lists it is possible to parallelize the
...
implemented as a plugable solution\fixme{another word for plugable?}, which can
be turned on and off at will. This has been achieved with the design found in
\refalg{alg:emptySpaceMaximizing}, which injects new nodes created by empty
