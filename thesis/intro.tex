...
\reffig{fig:gpuCpuCompare}. Unfortunatly, even with all this power certain
...
describtion can be found on \reffig{fig:cubemap}. The trouble with cube mapping
...
Another technique for rendering is \textit{ray tracing}, which eloquently solves
...
corrosponding tree can be seen on \reffig{fig:kdIteration} in
\refchapter{chp:kdTrees}, where kd-trees will be introduced thoroughly. If a
...
splitting plane for a node is the topic of \refsection{sec:splittingPlane}. 
...
%% Because of this, the algorithms for creating and restructuring kd-trees needs to
%% be very fast, and might even have to sacrifice tree quality for improved
%% construction speed.

Since this thesis is about efficiently rendering dynamic scenes, I will need to
...
choice of splitting plane in \refsection{sec:splittingPlane} and how geometry is
associated with child nodes after a split in
\refsection{sec:splittingSchemes}. In \refsection{sec:kdTreeImpl} I will discuss
how to implement kd-tree constructors efficiently on dataparallel GPUs using
CUDA.

The constructed kd-trees will be evaluated in \refchapter{chp:results} by how
...
In \refchapter{chp:rayTracing} I will discuss the ray tracers used to evaluate
...
entire \refchapter{chp:rayTracing} is devoted to discussing ray tracers and
...
question is whether or not we even need them. In \refsection{sec:exhaustive} I
...
%% described in \refsection{sec:emptySpace}. I shall investigate whether
